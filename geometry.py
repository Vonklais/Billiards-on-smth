...
def Calc_Angles(Th, fi, Alpha, D_Th, D_fi, D_Alpha, a, b, c, d):
...
#КОСТЫЛЬ НА 1 ДЕНЬ!
    ...
    """
    Rtt = 3 * R_Th * R_Th / r + 2 * ((r**3 / 2) * math.cos(2 * Th) * (1.0 / (c ** 2) - (CF ** 2) / (a ** 2) - (SF**2) * (CA ** 2) / (b ** 2) - (SF**2) * (SA ** 2) / (d ** 2)))
    Rtf = 3 * R_Th * R_fi / r + 2 * ((r ** 3 / 2) * (ST * CT) * math.sin(2 * fi) * (1.0 / (a ** 2) - (CA ** 2) / (b ** 2) - (SA ** 2) / (d ** 2) ))
    Rta = 3 * R_Th * R_Alpha / r + 2 * R_Alpha / math.tan(Th)
    Rff = 3 * R_fi * R_fi / r + 2 * R_fi / math.tan(2 * fi)
    Rfa = 3 * R_fi * R_Alpha / r + 2 * R_Alpha / math.tan(fi)
    Raa = 3 * R_Alpha * R_Alpha / r + 2 * R_Alpha / math.tan(2 * Alpha)
    ...
    if d != 0:
        Rtt = 3 * R_Th * R_Th / r + 2 * ((r**3 / 2) * math.cos(2 * Th) * (1.0 / (c ** 2) - (CF ** 2) / (a ** 2) - (SF**2) * (CA ** 2) / (b ** 2) - (SF**2) * (SA ** 2) / (d ** 2)))
        Rtf = 3 * R_Th * R_fi / r + 2 * ((r ** 3 / 2) * (ST * CT) * math.sin(2 * fi) * (1.0 / (a ** 2) - (CA ** 2) / (b ** 2) - (SA ** 2) / (d ** 2) ))
        Rta = 3 * R_Th * R_Alpha / r + 2 * ((r**3 / 2) * (ST * CT) * (SF ** 2) * math.sin(2 * Alpha) * (1.0 / (b ** 2) - 1.0 / (d ** 2) ))
        Rff = 3 * R_fi * R_fi / r + 2 * ((r ** 3 / 2) * (ST ** 2) * math.cos(2 * fi) * (1.0 / (a ** 2) - (CA ** 2) / (b ** 2) - (SA ** 2) / (d ** 2) ))
        Rfa = 3 * R_fi * R_Alpha / r + 2 * ((r**3 / 2) * (ST ** 2) * (SF * CF) * math.sin(2 * Alpha) * (1.0 / (b ** 2) - 1.0 / (d ** 2) ))
        Raa = 3 * R_Alpha * R_Alpha / r + 2 * ((r**3 / 2) * (ST ** 2) * (SF ** 2) * math.cos(2 * Alpha) * (1.0 / (b ** 2) - 1.0 / (d ** 2) ))
    else:
        Rtt = 3 * R_Th * R_Th / r + 2 * ((r ** 3 / 2) * math.cos(2 * Th) * (1.0 / (c ** 2) - (CF ** 2) / (a ** 2) - (SF ** 2) / (b ** 2)))
        Rtf = 3 * R_Th * R_fi / r + 2 * ((r ** 3 / 2) * (ST * CT) * math.sin(2 * fi) * (1.0 / (a ** 2) - 1.0 / (b ** 2)))
        Rta = 0
        Rff = 3 * R_fi * R_fi / r + 2 * ((r ** 3 / 2) * (ST ** 2) * math.cos(2 * fi) * (1.0 / (a ** 2) - 1.0 / (b ** 2)))
        Rfa = 0
        Raa = 0
    ...
    TTh = 2*D_r*(Rtt*D_Th + Rtf*D_fi + Rta*D_Alpha) + 2*r*R_Th*temp + r*r*math.sin(2*Th)*(D_fi*D_fi + math.sin(fi)*math.sin(fi)*D_Alpha*D_Alpha)
    Tfi = 2*D_r*(Rtf*D_Th + Rff*D_fi + Rfa*D_Alpha) + 2*r*R_fi*temp + r*r*math.sin(Th)*math.sin(Th)*math.sin(2*fi)*D_Alpha*D_Alpha
    TAlpha = 2*D_r*(Rta*D_Th + Rfa*D_fi + Raa*D_Alpha) + 2*r*R_Alpha*temp


    TThp_old = 2*D_r*R_Th + 2*r*r*D_Th
    Tfip_old = 2*D_r*R_fi + 2*r*r*math.sin(Th)*math.sin(Th)*D_fi
    TAlphap_old = 2*D_r*R_Alpha + 2*r*r*math.sin(Th)*math.sin(Th)*math.sin(fi)*math.sin(fi)*D_Alpha

    fi_New = fi + D_fi
    Th_New = Th + D_Th
    Alpha_New = Alpha + D_Alpha
    r_New = R_Calc(fi_New, Th_New, Alpha_New, a, b, c, d)

    R_Th_New = Rp_th(fi_New, Th_New, Alpha_New, r_New, a, b, c, d)
    R_fi_New = Rp_fi(fi_New, Th_New, Alpha_New, r_New, a, b, c, d)
    R_Alpha_New = Rp_Alpha(fi_New, Th_New, Alpha_New, r_New, a, b, c, d)


    q = np.array([[R_Th_New * R_Th_New + r_New * r_New, R_Th_New * R_fi_New, R_Th_New * R_Alpha_New],
                  [R_Th_New * R_fi_New, R_fi_New * R_fi_New + r_New * r_New * math.sin(Th_New) * math.sin(Th_New),R_fi_New * R_Alpha_New],
                  [R_Th_New*R_Alpha_New, R_fi_New*R_Alpha_New, R_Alpha_New * R_Alpha_New + r_New * r_New * math.sin(Th_New) * math.sin(Th_New) * math.sin(fi_New) * math.sin(fi_New)]])

    w = np.array([(TTh + TThp_old)/2, (Tfi + Tfip_old)/2, (TAlpha + TAlphap_old)/2])


    #ПРОВЕРИТЬ!!!
    D_Th_New, D_fi_New, D_Alpha_New = np.linalg.lstsq(q, w, rcond=None)[0]

    return Th_New, fi_New, Alpha_New, D_Th_New, D_fi_New, D_Alpha_New
...
def Sph_Lagrange_System(t, y, a, b, c, d, l, Constants = None):
    Th, fi, Alpha, D_Th, D_fi, D_Alpha = y
    if d == 0:
        Alpha, D_Alpha = 0.0, 0.0
    r = R_Calc(fi, Th, Alpha, a, b, c, d)
...
def PosCorrection3D(X, Y, Z, D_X, D_Y, D_Z, a, b, c, l,Check_reflection):
    if X**2/(a**2-l**2) + Y**2/(b**2-l**2) + Z**2/(c**2-l**2) > 1:
        temp = 1
    elif X**2/(a**2-l**2) + Y**2/(b**2-l**2) + Z**2/(c**2-l**2) < 1:
        temp = -1
    elif X**2/(a**2-l**2) + Y**2/(b**2-l**2) + Z**2/(c**2-l**2) == 1:
        return X,Y,Z
    dob = 1
    while(( X**2/(a**2-l**2) + Y**2/(b**2-l**2) + Z**2/(c**2-l**2))*temp > 0 and dob <=10):
        print(X,Y,Z)
        print(D_X, D_Y, D_Z)
        dob+=1
        X,Y,Z = X - D_X/(2.0**dob), Y - D_Y/(2.0**dob), Z - D_Z/(2.0**dob)
    return X,Y,Z




def Reflection3D(X, Y, Z, D_X, D_Y, D_Z, a, b, c, l,Check_reflection):
    ...
    cross_product = np.cross(n0, n1)
    # Норма (длина) вектора
    norm = np.linalg.norm(cross_product)
    # Нормализация вектора
    normalized_cross_product = cross_product / norm #Единичный вектор вдоль границы
    vel = np.array([D_X, D_Y, D_Z])  # вектор скорости
    s1 = np.dot(normalized_cross_product, vel)


    Vt = normalized_cross_product*s1
    Vel_finish = 2*Vt - vel
    print(vel)
    print(Vel_finish)
