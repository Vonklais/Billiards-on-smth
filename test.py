...
def Error_traj_calc_Sph_RK(fiin, dfiin, Thin, dThin, a, b, c, steps):
    ...
    y_full, t_full = RungeKut.RK_Lagrange_Calc(y0, a, b, c, 0, 0, t_span, t_eval_step, False)
...
def error_Calc_Sph(Axes_XYZ, x, y, z, H, trajnum, step_num):
    ...
    while (math.sin(Th) < math.sqrt(0.5)):
...
def plot_ell_Errors(a,b,c, girds_num, trajnum, step_num,H=1):
    ...
    cbar = fig.colorbar(mappable, ax=ax, shrink=0.6, aspect=10, label='Значение ошибки', ticks=[error_min, (error_min+error_max)/2, error_max])
    ...
    fig2.colorbar(t, ax=ax2, label='Значение ошибки')
